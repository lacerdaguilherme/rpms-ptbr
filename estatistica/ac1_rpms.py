...
#
# Mesma ideia do ac1_epmr.py, só que pro RPMS (escala respondida pelos
# juízes NÃO especialistas, os próprios refugiados que avaliaram os
# itens). Calcula SÓ o AC1 de Gwet, sem CVI junto (isso já existe em
# cvi_ac1_rpms.py) - a orientadora pediu explicitamente um produto com o
# AC1 isolado, já que é o único coeficiente que vai pro corpo do artigo
# (ver RESULTADOS_PARA_ARTIGO.md, seção 0).
#
# ATENCAO - isso aqui é importante, não pode esquecer (mesmo cuidado dos
# outros scripts do RPMS):
# as planilhas desse arquivo excel têm o NOME DE VERDADE dos juízes escrito
# no nome da aba (tipo "Juiz 1 (Fulano de Tal)") e mais pra baixo na planilha
# (da linha 28 em diante) tem até nome completo, idade, país de origem e
# outras informações desses juízes.
#
# Isso é dado sensível e NÃO PODE aparecer em lugar nenhum do resultado
# desse script. Por isso, aqui eu:
# 1) só leio da linha 6 até linha 26 de cada planilha (nunca mais que isso,
#    o resto eu nem toco)
# 2) NUNCA uso o nome real da aba/planilha em nada que é impresso ou salvo em
#    arquivo - cada juiz vira só "Juiz 1", "Juiz 2", "Juiz 3" e "Juiz 4", na
#    ordem que a aba aparece dentro do excel
#
# Como rodar:
#   uv run estatistica/ac1_rpms.py
#   uv run estatistica/ac1_rpms.py "caminho/outro_arquivo.xlsx"
#
...
# reclassificacao daria 100% "Sim" sem nenhuma variacao (AC1 indefinido,
# porque Pa = 1 e Pe = 0). Ver EXPLICACAO.md, secao 16.
...
def montar_texto_do_relatorio(matriz, numero_de_juizes):
    ...
    linhas = []
    linhas.append("=" * 60)
    linhas.append("RESULTADO - AC1 de Gwet - RPMS (juízes não especialistas)")
    linhas.append("=" * 60)
    linhas.append(
        "Número de juízes: "
    ...
    if math.isnan(valor_ac1):
        ...
        linhas.append(
            "  Sem nenhuma variação nas respostas, não sobra discordância nenhuma pra uma "
            "estatística de concordância corrigida por acaso medir."
        )
    else:
        linhas.append("Concordância esperada ao acaso pelo AC1 (Pe): " + str(round(pe, 4)))
...
def salvar_relatorio_em_arquivo(texto_relatorio, agora):
    ...
    nome_do_arquivo = "ac1_rpms_" + agora + ".txt"
...
def montar_resumo_para_figura(matriz, numero_de_juizes):
    ...
    ac1_epmr.py pra explicação completa. Mesma lógica aqui, só muda o
    título e os juízes serem identificados como "não especialistas".
    """
    ...
    titulo = "AC1 de Gwet: RPMS (juízes não especialistas)"

    if math.isnan(valor_ac1):
        notas = [
            "AC1 indefinido: sem nenhuma variação nas respostas, não sobra discordância nos "
            "dados pra uma estatística corrigida por acaso medir (ver EXPLICACAO.md, seção 9)."
        ]
    else:
...
def salvar_imagem_relatorio(resumo, agora):
    ...
    nome_do_arquivo = "ac1_rpms_" + agora + ".png"
    caminho_completo = PASTA_RESULTADOS / nome_do_arquivo
...
def salvar_html_relatorio(resumo, nome_da_imagem, agora):
    ...
    nome_do_arquivo = "ac1_rpms_" + agora + ".html"
    caminho_completo = PASTA_RESULTADOS / nome_do_arquivo
